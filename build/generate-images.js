...
function loadPrompts(slug) {
  ...
  if (!cfg.prompts || !Object.keys(cfg.prompts).length) throw new Error(f + ': no "prompts"');
  return { style: cfg.style || STYLE, anchor: String(cfg.anchor || Object.keys(cfg.prompts)[0]), prompts: cfg.prompts };
}
...
(async () => {
  ...
  if (mode === 'anchor' || mode === 'batch') {
    const slug = process.argv[3];
    const { style, anchor, prompts } = loadPrompts(slug);
    const OUT = outDir(slug);
    fs.mkdirSync(OUT, { recursive: true });
    console.log('output dir:', OUT);
    const anchorFile = path.join(OUT, 'anchor.png');

    if (mode === 'anchor') return generate(prompts[anchor], style, null, anchorFile);

    if (!fs.existsSync(anchorFile)) throw new Error('no anchor yet - run: node build/generate-images.js anchor ' + slug);
    ...
    for (const [n, prompt] of Object.entries(prompts))
      for (let c = 0; c < per; c++) jobs.push({ n, c, prompt });
